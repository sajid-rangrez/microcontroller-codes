...
String city = "Bangalore"; //city name
const char* countryCode = "IN"; //country code
String apiKey = "4031e77dcdd6900f0d43e0274646be09"; //openweathermap api
...
ForecastDay fcast[3];
const char* ssid = "Sajid's Galaxy S23"; // ssid
const char* password = "SAJID12345";  //password
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (JSON.typeof(myObject) != "undefined") {
        ...
        // Safe string manipulation for the description
        String d = (const char*)myObject["weather"][0]["description"];
        if (d.length() > 0) {
          d[0] = toupper(d[0]);
          weatherDesc = d;
        }
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (JSON.typeof(fo) != "undefined") {
        // ESP8266 Time Handling
        time_t now = time(nullptr);
        struct tm *t = localtime(&now);
        int today = t->tm_wday; 

        const char* days[] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
        int indices[3] = { 7, 15, 23 }; // 24h, 48h, 72h jumps roughly
        
        for (int i = 0; i < 3; i++) {
          int idx = indices[i];
          if (fo["list"][idx].hasOwnProperty("main")) {
...
void playBootAnimation() {
  ...
  display.setFont(&FreeSansBold9pt7b);
  String bootText = "machine.bug64bit";
